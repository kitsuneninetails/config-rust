...
fn test_get_scalar_path() {
    ...
    assert_eq!(c.get("place.creators[0].name").ok(), Some("John Smith".to_string()));
}
...
fn test_config_tree() {
    let c = make();
    let m: Config = c.get_tree("place").unwrap();
    assert_eq!(m.get_str("name").unwrap(), "Torre di Pisa".to_string());
    assert_eq!(m.get_int("reviews").unwrap(), 3866);
}

#[test]
fn test_map_str() {
    let c = make();
    let m: HashMap<String, String> = c.get("place.creators[0]").unwrap();

    assert_eq!(m.len(), 2);
    assert_eq!(m["name"], "John Smith".to_string());
    assert_eq!(m["id"], "12345".to_string());
}
